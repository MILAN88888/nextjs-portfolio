...
- **`role` is written from `git log`, not from memory.** Before changing one, count the commits and read the subjects:
  `git -C <plugin> log --author=milan --format='%s' | grep -viE '^(merge|bump|version)'`
  The current lines came from that pass: SmartSMTP is 303 of 331 commits his (hence "built it out through its first releases"), Customize My Account for WooCommerce 158, Registration Form Fields 68, Everest Forms 182, User Registration 99 plus 20+ add-ons. Vague roles like "block work" or "mailer routing" understate him — name the field, the module or the fix.
...
- **Long card content folds.** `GridProject` shows the first description paragraph, then puts any further paragraphs and a `role` over 170 characters behind `ui/Expandable`. That primitive is a native `<details>` styled by `.disclosure` in `globals.css`, so it needs no client JavaScript and stays a server component — don't rewrite it with `useState`. The featured card never folds: it is the one meant to be read in full.
