...
- **Exactly one entry carries `featured: true`** — the AI gateway. It renders full width at the top of the grid as prose with a footer row for stack and links. No bullet list: Milan asked for it in sentences. `Projects.tsx` has two render paths (`FeaturedProject`, `GridProject`) sharing the same small parts; keep new fields flowing through both.
